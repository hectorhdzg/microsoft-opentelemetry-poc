...
> **Status (March 2026):** All infrastructure is in place. Bot endpoint registration
> with the A365 service completed successfully. Waiting for the agent to appear in
> Teams Agent Store (propagation can take up to 15 minutes after endpoint registration).
...
| A365 CLI | `npm install -g @anthropic/a365-cli` — v1.1.109-preview used here |
...
- Blueprint App ID: `a9cd647c-c40d-4f95-8195-11ad34d6c7b6`
- Blueprint display name: `hectorhtestagentv2 Blueprint`
- Created via: Developer Portal (CLI creation had issues with the first attempt)
...
- SP Object ID: `120e054f-bddf-4deb-b32d-93ecd11ed301`
- Client secret: stored in `.env` and Azure App Settings (not committed)
- FIC: created against the SP object ID
...
```bash
az webapp config appsettings set --name hectorhtestagent-webapp \
...
```bash
az webapp deploy --name hectorhtestagent-webapp \
  --resource-group rg-hectorh \
  --src-path <zip-file> \
  --type zip
```
...
```bash
# Health check
curl https://hectorhtestagent-webapp.azurewebsites.net/api/health

# Messaging endpoint (should return 401/auth error, NOT 404)
curl https://hectorhtestagent-webapp.azurewebsites.net/api/messages
...
```json
{
  "botId": "a9cd647c-c40d-4f95-8195-11ad34d6c7b6",        // was null
  "botMsaAppId": "a9cd647c-c40d-4f95-8195-11ad34d6c7b6",   // was null
...
The manifest (`manifest/manifest.json`) needs these sections for Teams + Copilot:
    ...
    "customEngineAgents": [
      {
        "id": "<blueprint-app-id>",
        "type": "bot",
        "functionsAs": "agentOrAgenticUser",
...
Key points:
- **`bots` section** — required for the Teams channel to work
- **`copilotAgents.customEngineAgents`** — required for Copilot integration
- **`manifestVersion: "devPreview"`** — required for these features
- The `botId` must match your blueprint app ID

### Upload to Admin Center

1. Zip the `manifest/` folder (must contain `manifest.json`, `color.png`, `outline.png`)
2. Go to [Teams Admin Center](https://admin.teams.microsoft.com) → Teams apps → Manage apps
3. Click **Upload new app** → **Upload custom agent** (NOT "Upload custom app")
4. Upload the zip
...
- Verify manifest has `bots` section with correct `botId`
